		     ...
		     INTER *inter,NGBR *ngbr,COORDS *coords,STARTUP *startup)
{
  ...
  for(i=0,key_now=key_root;i<num_keys;i++,key_now=key_now->next){
	...
	strcpy(line,"Calculation type must be md, subspace, or NMA not ");
	...
	sprintf(line,"ensemble must be nve,nvt,npt_{i,l,f} or min != %s",
	       key_now->keyarg);
	md_error(line);
      }
      break;
	...
	sprintf(line,"Allowed arguements for %s are \"on\" or \"off\"",
		key_now->keyword);
	md_error(line);
      }
      break;
    ...
    case 40: /*nres_lrf */
      sscanf(key_now->keyarg,"%d",&(simparms->ninner));
      if(simparms->ninner<=0) md_error("nres_lrf must be > 0");
      break;
    case 41: /*nres_intra*/
    ...
    case 80:/*nres_nhc*/ 
      ...
      break;
    case 81:/*nres_nhc*/ 
      strcpy(simparms->yoshidas,key_now->keyarg); 
      break;
    case 82: /* scaleeps */
  ...
  if(simparms->icalc_type==0){
    ...
    if( simparms->ninner <= 0 || simparms->ninnra <= 0){
  ...
  if(simparms->icalc_type==1){
    ...
    if( simparms->ninner > 1 || simparms->ninnra > 1){
