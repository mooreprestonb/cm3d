...
void set_baro_new(SIMPARMS *simparms,COORDS *coords,double tauv)
{
  ...
  sprintf(line,"Barostat set up: mvol = %g and you have %d barostates",
     coords->mvol,simparms->nbar);
  md_stdout(line);
}
/*----------------------------------------------------------------*/
...
void intbar_new(SIMPARMS *simparms,COORDS *coords,double dta2,int iflag)
{
  ...
  switch(iflag){
     case 0:
       app_ux2(simparms->natoms,dta2,coords);
       app_up2(simparms->natoms,dta2,dnf,coords);
       app_uh(dta2,coords);
       
       /* Volume Chain */
       if(simparms->nbar>0){
         expvV = exp(-dta2*coords->vbar[0]);
#ifdef ISO
         coords->vvol *= expvV;
#endif
         for(i=0;i<9;i++) coords->vvol9[i] *= expvV;
         for(i=0;i<simparms->nbar-1;i++){
           coords->vbar[i] *= exp(-dta2*coords->vbar[i+1]);
         }
         for(i=simparms->nbar-1;i>=1;i--) {
           coords->vbar[i] += dta2*(coords->mbar[i-1]*coords->vbar[i-1]*
              coords->vbar[i-1]-simparms->temp)/coords->mbar[i];
         }
#ifdef ISO
         coords->vbar[0] += (dta2*(3.*coords->mvol*coords->vvol*coords->vvol-
            simparms->temp)/coords->mbar[0]);
#else
         expvV = 0.;
         for(i=0;i<9;i++) expvV += coords->vvol9[i]*coords->vvol9[i];
         coords->vbar[0] += (dta2*(coords->mvol*expvV-simparms->temp)/
            coords->mbar[0]);
#endif
         for(i=0;i<simparms->nbar;i++)coords->bar[i] += dta2*coords->vbar[i];
       }
       break;
     case 1:
       if(simparms->nbar>0){
