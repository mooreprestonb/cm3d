  ...
  for(imkey=0;imkey<nmeta_key && ifound==0;imkey++,meta_now=meta_now->next){
    if(!strncasecmp(META_KEY_INTER,meta_now->meta,strlen(META_KEY_INTER))){
	  ...
	  default:
        ...
        if(!(!strcasecmp(POT_TYPE_NULL,pot_type) ||
	     ...
	     !strcasecmp(POT_TYPE_LJ124,pot_type) || 
	  ...
	  /* right now we are just using sig and eps, like the LJ pot. */
	  for(i=6;i<8;i++){ 
	    if(num_found[i] != 1)
              err_found(dict[i],num_found[i],filename,nline);
          }
	...
	*/
	    ...
	    */
          ...
          if(!strcasecmp(POT_TYPE_LJ124,pot_type))
            func_lj124(sqrt(xt),&f,&df,&d2f,eps_now,sig_now);
          if(!strcasecmp(POT_TYPE_WILL,pot_type))
...

{
  ...
  for(imkey=0;imkey<nmeta_key && ifound==0;imkey++,meta_now=meta_now->next){
	...
	case 8: sscanf(key_now->arg,"%lg",&(a0[0])); break;/* a0 */
	case 9: sscanf(key_now->arg,"%lg",&(a0[1])); break;/* a1 */
	case 10: sscanf(key_now->arg,"%lg",&(a0[2])); break;/* a2 */
	case 11: sscanf(key_now->arg,"%lg",&(a0[3])); break;/* a3 */
	case 12: sscanf(key_now->arg,"%lg",&(a0[4])); break;/* a4 */
	default:
      ...
      else if (!strcasecmp(POT_TYPE_QUARTIC,pot_type)) ptype = 2;
      else {
        fprintf(stderr,"ERROR: bond pot_type %s (in file %s not implimented\n",
		pot_type,filename);
        exit(1);
      }
      /* harmonic type ... zero morse and quartic */
      if(ptype==0) {
	for(i=3;i<=4;i++){
	  if(num_found[i] != 1){
	    err_found(dict[i],num_found[i],filename,nline); 
	...
	if(eq<0 || fk<0){
	  sprintf(line,"negative force(%g) or eq(%g) in bond harm with %s and %s",
		  fk,eq,atom1,atom2);
	  md_error(line);
	}
	bm = 0.;
	for(i=0;i<5;++i) a0[i] = 0;
      }
      /* morse type check for bmorse */
      if(ptype==1 ){
        if(num_found[7]==0) err_found(dict[7],num_found[7],filename,nline);
	for(i=3;i<=4;i++){
	  if(num_found[i] != 1){
	    err_found(dict[i],num_found[i],filename,nline); 
	  }  
	...
	if(eq<0 || fk<0 || bm<0){
	  sprintf(line,"negative force(%g) or eq(%g) or b(%g) in bond morse with %s and %s",
		  fk,eq,bm,atom1,atom2);
	  md_error(line);
	}
      }
      ...
      if(ptype==2){
	bm = 0.;
	if(num_found[3] == 0) eq = 0;
	if(num_found[4] == 0) fk = 0;
	for(i=8;i<13;++i){
	  if(num_found[i]==0){
	    err_found(dict[i],num_found[i],filename,nline);
	  }
	...
	for(i=0;i<NP_BOND;++i) a0n[i] = a0[i];
      }
    }
  }
  if(!ifound){
		   ...
		   double *fk_now,int *ifix)
{
  ...
  while((ich = fgetc(fp)) != EOF && !(ifound)) {
	...
	if(eq1<0 || eq2<0 || fk<0){
	  sprintf(line,"negative force(%g) or eq1(%g) or eq2(%g) in bondx with %s and %s and %s",
		  fk,eq1,eq2,atom1,atom2,atom3);
	  md_error(line);
	}
	...
	if(!strcasecmp(POT_TYPE_BONDX,pot_type)){
	  ...
	  fprintf(stderr,"ERROR: bondx pot_type %s (in file %s) not implimented\n",
		  pot_type,filename);
	  exit(1);
	}
      }
...
void get_bendparm(FILE *fp,char *filename,WORD atm1,WORD atm2,WORD atm3,
		  int *it_now, WORD bend_label,double *eq_now,double *fk_now,
		  int *ifix,int ibend,int jbend,int kbend,double *aq)
{
  char line[MAXLINELEN];
  int i,ifound,ich,nline,nch,num_keys,icase,ifx,ptype;
  int num_dict=13,num_found[13];
  WORD dict[13];
  WORD metakey,atom1,atom2,atom3,pot_type,tword,label;
  KEY key_root,*key_now;
  double eq,fk,acn[NP_BEND];
  ...
  while( (ich = fgetc(fp)) != EOF && !(ifound)) {
	...
	for(i=0;i<NP_BEND;++i) acn[i] = 0;
	  ...
	  case 8:sscanf(key_now->keyarg,"%lg",&(acn[0]));break;	/* a0 */
	  case 9:sscanf(key_now->keyarg,"%lg",&(acn[1]));break;	/* a1 */
	  case 10:sscanf(key_now->keyarg,"%lg",&(acn[2]));break;/* a2 */
	  case 11:sscanf(key_now->keyarg,"%lg",&(acn[3]));break;/* a3 */
	  case 12:sscanf(key_now->keyarg,"%lg",&(acn[4]));break;/* a4 */
	  default:
	    print_dict(key_now->keyword,num_dict,dict); exit(1);break;
          }
        }
	...
	if(!strcasecmp(atom1,atm1) && !strcasecmp(atom2,atm2) &&
	  ...
	  /* if match the ones we are looking for*/
	  if(!strcasecmp(POT_TYPE_HARM,pot_type)) ptype = 0;
	  else if (!strcasecmp(POT_TYPE_COSINE,pot_type)) ptype = 1;
	  else if (!strcasecmp(POT_TYPE_QUARTIC,pot_type)) ptype = 2;
	  else {
	    fprintf(stderr,"ERROR: bend pot_type %s (in file %s not implimented)\n",
		    pot_type,filename);
	    exit(1);
	  }
	  ...
	  case 0: /* harmonic */
	    ...
	    if(eq<0 || fk<0){
	      sprintf(line,"negative force(%g) or eq(%g) in bend harm with %s and %s and %s",
		      fk,eq,atom1,atom2,atom3);
	      md_error(line);
	    }
	    *eq_now = eq; *fk_now = fk; *ifix = ifx;
	    *it_now = 0; /* harmonic */
	    break;
	  case 1: /* cubic */
	    if(num_found[5] != 1){
	      err_found(dict[5],num_found[5],filename,nline); 
	    }
	    if(eq<0 || fk<0){
	      sprintf(line,"negative force(%g) or eq(%g) in bend cubic with %s and %s and %s",
		      fk,eq,atom1,atom2,atom3);
	      md_error(line);
	    }
	    ifound = 1; *eq_now = eq; *fk_now = fk; *ifix = ifx;
	    *it_now = 1; /* cubic */
	    break;
	  case 2: /* quartic */
	    for(i=8;i<8+NP_BEND;++i){
	      if(num_found[i] != 1){
		err_found(dict[i],num_found[i],filename,nline); 
	      }
	    }   
	    if(eq<0 || fk<0){
	      sprintf(line,"negative force(%g) or eq(%g) in bend quartic with %s and %s and %s",
		      fk,eq,atom1,atom2,atom3);
	      md_error(line);
	    }
	    for(i=0;i<NP_BEND;++i) aq[i] = acn[i];
	    ifound = 1; *eq_now = eq; *fk_now = fk; *ifix = ifx;
	    *it_now = 2; /* quartic */
	    break;
	  default:
  ...
  if(!ifound){
    fprintf(stderr,
	    "ERROR: Bend interaction between %s (%d) %s (%d) %s (%d) (%s) not found in %s\n"
  ...
  while( (ich = fgetc(fp)) != EOF && !(ifound)) {
    ...
    if(ich == '~'){
	  ...
	  /* harmonic do we have everything for this type? */
	    ...
	    if(eq<0 || fk<0){
	      sprintf(line,"negative force(%g) or eq(%g) in torsion harm with %s-%s-%s-%s",
		      fk,eq,atom1,atom2,atom3,atom4);
	      md_error(line);
	    }
            *int_pot = 0; *eq_now = eq; *fk_now = fk; 
          }
	  /* power series do we have everything for this type? */
	  ...
	  /* cosine /w phase, do we have everything for this type? */
          if(!strcasecmp(POT_TYPE_COSINE,pot_type)){
            for(i=7;i<=13;i++){
              if(num_found[i] != 1){
#ifdef ALLPOWER
                err_found(dict[i],num_found[i],filename,nline); 
	  ...
	  *ifix = ifx; /* are we trying to fit this function? */
        }
      }
      free_key(num_keys,&key_root);
    }
