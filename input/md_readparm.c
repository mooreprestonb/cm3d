		   ...
		   INTER *inter,NGBR *ngbr)
{
  ...
  tybe = (int **)cmalloc(startup->nspec*sizeof(int*));
  ...
  read_top_file(simparms,startup,atom_topol,
		ibo,jbo,tbo,ibe,jbe,kbe,tybe,tbe,
  ...
  simparms->itype = (int *)cmalloc(5*natoms*sizeof(int));
  simparms->iatom     = simparms->itype +   natoms;
  simparms->imolecule = simparms->itype + 2*natoms;
  simparms->ispecies  = simparms->itype + 3*natoms;
  simparms->igroup    = simparms->itype + 4*natoms;
  ...
  simparms->mem_bytes += natoms*(5*sizeof(int)+2*sizeof(WORD));
  simparms->mem_bytes += startup->nspec*sizeof(WORD);
  ...
  for(ispec=0,spec_now=&startup->spec_root;ispec < startup->nspec;
	...
	simparms->itype[ioff]     = i;
	simparms->iatom[ioff]     = j;
	simparms->imolecule[ioff] = k;
	simparms->ispecies[ioff]  = ispec;
	simparms->igroup[ioff]    = ngpm+ngroup+k;
	(coords->amass)[ioff] = atom_topol[ispec][j].mass*MCONV;
	(coords->qch)[ioff]   = (atom_topol[ispec][j].charge*sqrt(CCONV)*
				 simparms->scalecharge);
  ...
  for(i=0;i<natoms;i++){
    printf("%d %d %d %d %d %d %s %s %s %lg %lg\n",
	   i,simparms->ispecies[i],simparms->imolecule[i],simparms->igroup[i],
	   simparms->iatom[i],simparms->itype[i],
	   (label->mole)[simparms->ispecies[i]],
	   (label->atom)[simparms->itype[i]],
	   (label->group)[simparms->igroup[i].group],
	   (coords->amass)[i],(coords->qch)[i]);
  }
#endif

  simparms->ntypes = inter->ntype = ntypes;
  if(simparms->rank==0){
  ...
  for(i=0;i<natoms;i++){inter->itype[i] = simparms->itype[i];}

  /* search the data base for the interactions */
  search_ter_base(simparms,startup,inter,ngbr);
  ...
  search_bond_base(simparms,startup->bond_file,ibo,jbo,tbo,
		   ...
		   &coords->bonds);
  search_bend_base(simparms,startup->bend_file,ibe,jbe,kbe,tybe,tbe,atom_topol,
  ...
  for(i=0;i<startup->nspec;i++){
    ...
    if(ibe[i]!=NULL){
      free(ibe[i]); free(jbe[i]); free(kbe[i]); free(tybe[i]); free(tbe[i]);
    }
    /* if(ibeq[i]!=NULL){free(ibeq[i]); free(jbeq[i]); free(kbeq[i]); free(tbeq[i]);} */
     if(ito[i]!=NULL){
  ...
  free(ibe); free(jbe); free(kbe); free(tbe); free(tybe);
  /*  free(ibeq); free(jbeq); free(kbeq); free(tbeq); */
  free(ito); free(jto); free(kto); free(lto); free(tto);
  if(iof != NULL) free(iof); if(jof != NULL) free(jof);
		   ...
		   int **ibonx,int **jbonx,int **kbonx, WORD **tbonx)
{
  ...
  for(ispec=0;ispec<startup->nspec;ispec++,spec_now=spec_now->next){
    ...
    if (fp == NULL){
      sprintf(line,"can't open set file (%s)",spec_now->filename);
      md_error(line);
      exit(1);
    } else {
      if(simparms->rank==0){
	sprintf(line,"Reading in parameter from file %s",spec_now->filename);
	md_stdout(line);
      }
    }
    ...
    tybe[ispec] = NULL; tbe[ispec] = NULL; 
    if(spec_now->nbend>0){
      ...
      tybe[ispec] = (int *)cmalloc(spec_now->nbend*sizeof(int));
      tbe[ispec] = (WORD *)cmalloc(spec_now->nbend*sizeof(WORD));
      get_bend(fp,spec_now->filename,spec_now->nbend,
	       ibe[ispec],jbe[ispec],kbe[ispec],tybe[ispec],tbe[ispec]);
